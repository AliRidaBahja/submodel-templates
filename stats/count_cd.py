#!/usr/bin/env python3
"""
Count ConceptDescription ID prefixes with URL host-only namespaces.
...
Input: JSON array with objects containing "conceptDescription_id".
...
def count_prefixes(items: List[Dict[str, Any]], include_other: bool) -> Tuple[Counter, Dict[str, Counter]]:
    ...
    for it in items:
        cid = it.get("conceptDescription_id", "")
        kind, pref = extract_prefix(cid)
...
def parse_args():
    ap = argparse.ArgumentParser(description="Count ConceptDescription ID prefixes with URL host-only namespaces.")
